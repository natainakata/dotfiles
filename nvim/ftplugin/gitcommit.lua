local uv = vim.uv
local git_root = vim.fn.systemlist("git rev-parse --show-toplevel")[1] or ".git/hooks"
...
vim.b.is_precommit_pass = not tb(vim.fn.executable(pre_commit_path))
vim.b.is_commit_msg_pass = not tb(vim.fn.executable(commit_msg_path))

local cquit_if_not_passed = function()
  if not vim.b.is_precommit_pass or not vim.b.is_commit_msg_pass then
    vim.cmd.cquit()
  end
end
  ...
  if tb(vim.fn.executable(pre_commit_path)) then
    ...
    vim.fn.termopen("cd " .. git_root .. "&&" .. pre_commit_path, {
      on_exit = function(_, exit_code, _)
        if exit_code ~= 0 then
          vim.notify("pre-commit failed", vim.log.levels.EROOR)
        else
          vim.notify("pre_commit success", vim.log.levels.INFO)
        end
        vim.b.is_precommit_pass = exit_code == 0
      end,
    })
...
vim.api.nvim_create_autocmd("BufWritePost", {
  buffer = commit_bufnr,
  callback = function()
    if not tb(vim.b.is_precommit_pass) then
